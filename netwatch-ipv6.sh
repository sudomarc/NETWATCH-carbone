...
check_deps(){
    ...
    if [[ "$1" == control ]] && ! command -v ip6tables >/dev/null 2>&1; then
        die 'ip6tables is required for IPv6 blocking.'
    fi
}
...
scan(){
    ...
    # ff02::1 is the IPv6 link-local all-nodes multicast address.
    # The interface scope is supplied explicitly so discovery stays on-link.
    if command -v ping >/dev/null 2>&1; then
        ping -6 -c 1 -W 2 -I "$IFACE" ff02::1 >/dev/null 2>&1 || true
    fi
    ...
    ip -6 neigh show dev "$IFACE" 2>/dev/null \
        | awk '$1 ~ /^[0-9a-fA-F:]+$/ && $2 != "FAILED" && $2 != "INCOMPLETE" {print}' > "$tmp"

    case "$format" in
        json)
            printf '[\n'
            local first=true
            while read -r ipaddr _ state _ mac _; do
                [[ -z "$ipaddr" ]] && continue
                $first || printf ',\n'
                first=false
                printf '  {"ipv6":"%s","mac":"%s","state":"%s"}' "$ipaddr" "${mac:---}" "${state:--}"
            done < "$tmp"
            printf '\n]\n'
            ;;
        csv)
...
block(){
    ...
    [[ "$(cat /proc/sys/net/ipv6/conf/all/forwarding 2>/dev/null || printf 0)" == 1 ]] \
        || die 'IPv6 blocking requires this Linux host to be the IPv6 router/gateway (forwarding=1).'

    mkdir -p "$CONFIG_DIR"
    if $DRY_RUN; then
        warn "[DRY-RUN] ip6tables would add DROP rules for $target"
        return 0
    fi
